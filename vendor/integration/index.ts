  ...
  return {
    ...
    hooks: {
      'astro:config:setup': async ({
        ...
        const buildLogger = logger.fork('neko-hi');

        const virtualModuleId = 'neko-hi:config';
        ...
        updateConfig({
          ...
          vite: {
            plugins: [
              {
                name: 'vite-plugin-neko-hi-config',
                resolveId(id) {
        ...
        if (typeof _themeConfig === 'string') {
          ...
          buildLogger.info(`neko-hi \`${_themeConfig}\` has been loaded.`);
        } else {
          buildLogger.info(`neko-hi config has been loaded.`);
        }
      },
